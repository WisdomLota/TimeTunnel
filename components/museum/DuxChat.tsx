...
export default function DuxChat({
  ...
  async function doSend(text: string) {
    ...
    try {
      ...
      while (true) {
        ...
        const copy = [...updated, { role: "assistant" as const, content: assistantText }];
      ...
      const errMsg = { role: "assistant" as const, content: lang === "en" ? "Sorry, I lost my train of thought. Try again?" : "Üzgünüm, düşüncemi kaybettim. Tekrar dener misiniz?" };
      const copy = [...messagesRef.current, errMsg];
      messagesRef.current = copy;
      setMessages([...copy]);
    } finally {
  ...
  return (
    <motion.div className="flex flex-col w-full max-w-sm mx-auto" style={{ height: "100%" }} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
      {/* Header */}
      ...
      <div className="px-4 py-3">
        <AnimatePresence mode="wait">
          {recording ? (
            <motion.div key="rec" className="flex items-center gap-3 px-3 py-2.5 rounded-lg" style={{ background: `${accentColor}10`, border: `1px solid ${accentColor}33` }} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}>
              ...
              <button onClick={() => stopRec(true)} className="p-1.5 rounded-full shrink-0" style={{ color: `${accentColor}66` }}>
              ...
              <button onClick={() => stopRec(false)} className="p-2 rounded-full shrink-0" style={{ background: accentColor }}>
